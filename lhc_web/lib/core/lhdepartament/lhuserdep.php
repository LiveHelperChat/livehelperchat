...
class erLhcoreClassUserDep
{
    ...
    public static function addUserDepartaments($Departaments, $userID = false, $UserData = false, $readOnly = array())
    {
        ...
        foreach ($Departaments as $DepartamentID) {
            $stmt = $db->prepare('INSERT INTO lh_userdep (user_id,dep_id,hide_online,last_activity,last_accepted,active_chats,type,dep_group_id,max_chats,ro) VALUES (:user_id,:dep_id,:hide_online,0,0,:active_chats,0,0,:max_chats,:ro)');
            $stmt->bindValue(':user_id', $userID);
