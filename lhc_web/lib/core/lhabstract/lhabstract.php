        ...
        switch ($attr['type']) {
            ...
            case 'checkbox_multi':
                ...
                foreach ($items as $item) {
                    ...
                    $nameAttr = isset($attr['name_attr']) ? $item->{$attr['name_attr']} : ((string)$item);
                    $return .= '<div ng-non-bindable class="col-' . $attr['col_size'] . '"><label><input class="me-1 form-check-input" type="checkbox" name="AbstractInput_' . $name . '[]" ' . $selected . ' value="' . $item->id . '">' . htmlspecialchars($nameAttr) . '</label></div>';
