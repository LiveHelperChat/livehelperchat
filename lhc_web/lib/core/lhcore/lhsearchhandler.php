...
class erLhcoreClassSearchHandler {
    ...
    /*public static function renderFrontendSearchParams($searchParams,$moduleParams)
    {
        $fieldsObjects = include('lib/core/lh'.$moduleParams['module'].'/searchattr/'.$moduleParams['file'].'.php');
   ...
   public static function isContactProvided($text) {
   		...
   		/* if (preg_match('#(?<!=[\'"])(?<=[*\')+.,;:!&$\s>])(\()?([\w]+?://(?:[\w\\x80-\\xff\#%~/?@\[\]-]|[\'*(+.,;:!=&$](?![\b\)]|(\))?([\s]|$))|(?(1)\)(?![\s<.,;:]|$)|\)))+)#is', $text)) {
   			return true;
   		} */
   		
    	/* if (preg_match('#([\s>])((www|ftp)\.[\w\\x80-\\xff\#$%&~/.\-;:=,?@\[\]+]+)#is', $text)){
    		return true;
    	} */
    	...
    	/* if (self::detectPhone($text) || self::detectPhone(str_replace(' ', '', $text)) || self::detectPhone(preg_replace('/(?<=\d)\s+(?=\d)/',"", $text))){
    		return true;
    	} */
