    ...
    public static function closeChat($params)
    {
        if ($params['chat']->status != erLhcoreClassModelChat::STATUS_CLOSED_CHAT) {
            ...
            $db->beginTransaction();
                ...
                $params['chat']->chat_duration = \LiveHelperChat\Helpers\ChatDuration::getChatDurationToUpdateChatID($params['chat']);
                $params['chat']->has_unread_messages = 0;
    ...
    public static function changeStatus($params)
    {
        ...
        if ($changeStatus == erLhcoreClassModelChat::STATUS_ACTIVE_CHAT) {
            ...
            if ($chat->status != erLhcoreClassModelChat::STATUS_CLOSED_CHAT) {
                ...
                $chat->chat_duration = \LiveHelperChat\Helpers\ChatDuration::getChatDurationToUpdateChatID($chat);
                $chat->cls_time = time();
