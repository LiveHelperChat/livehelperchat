...
class erLhcoreClassGenericBotActionRestapi
{
                ...
                if ($quickReplyButton['type'] == 'trigger' || $quickReplyButton['type'] == 'button') {
                    if (isset($matchCycles[1][0])) {
                        ...
                        $buttonsArray[] = str_replace(['{{button_payload}}','{{button_title}}'],[
                            json_encode(($quickReplyButton['type'] == 'button' ?  'bpayload__' : 'trigger__') . substr(md5($quickReplyButton['content']['payload']),0,16). '__' . substr(md5($quickReplyButton['content']['name']),0,16) .'__'.$paramsCustomer['params']['msg']->id),
                ...
                foreach ($matchesValues[0] as $indexElement => $elementValue) {
                    ...
                    foreach ($messages as $message) {
                        ...
                        if ($message->user_id == -1 && isset($message->meta_msg_array['content']['attr_options']['as_json']) && $message->meta_msg_array['content']['attr_options']['as_json'] == true) {
                            ...
                            // add parsing for [chatfilebody=2011_a70588f4754bbff64ce3a741bb18fbe3]
                            $matches = array();
                            preg_match_all('/\[chatfilebody="?(.*?)"?\]/', $message->msg, $matches);
                            ...
                            foreach ($matches[1] as $index => $body) {
                                ...
                                try {
                                    $file = erLhcoreClassModelChatFile::fetch($fileID);
                                    if (is_object($file) && $hash == $file->security_hash) {
                                        $fileBody = 'data:'.$file->type.';base64,'.base64_encode(file_get_contents($file->file_path_server));
    ...
    public static function processMultiStepUpload($mediaFile, &$methodSettings, $host, $headers, &$replaceVariables, &$replaceVariablesJSON, &$queryArgs, &$errorMessage, &$errorDetails = [])
    {
        ...
        $fileType = 'file';
        if (strpos($mediaFile->type, 'image/') === 0 || in_array(strtolower($mediaFile->extension), ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'])) {
            $fileType = 'image';
        } elseif (strpos($mediaFile->type, 'audio/') === 0 || in_array(strtolower($mediaFile->extension), ['mp3', 'ogg', 'oga', 'wav', 'm4a', 'aac', 'opus'])) {
            $fileType = 'audio';
        } elseif (strpos($mediaFile->type, 'video/') === 0 || in_array(strtolower($mediaFile->extension), ['mp4', 'mov', 'avi', 'mkv', 'webm'])) {
            $fileType = 'video';
        }
