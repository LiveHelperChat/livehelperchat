...
<?php foreach ($transferchats as $chat) : ?>
              ...
              <img class="action-image" align="absmiddle" data-title="<?php echo erLhcoreClassDesign::shrt($chat['nick'],10,'...',30,ENT_QUOTES);?>" onclick="lhinst.startChatNewWindowTransfer('<?php echo $chat['id'];?>',$(this).attr('data-title'),'<?php echo $chat['transfer_id'];?>')" src="<?php echo erLhcoreClassDesign::design('images/icons/application_add.png');?>" alt="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Open in a new window');?>" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Open in a new window');?>"> <?php echo $chat['id'];?>. <?php echo htmlspecialchars($chat['nick']);?> (<?php echo date('Y-m-d H:i:s',$chat['time']);?>)
              </li>
<?php endforeach; ?>
</ul>
<?php } else { ?>
