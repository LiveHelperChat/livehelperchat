                        ...
                        <div class="lhc-item-list mb-2">
                            <div class="lhc-item-list-title fs13">
                               <?php if ($item['type'] == 'webhook') : ?>
                                   <i class="material-icons">webhook</i><a class="me-1" href="<?php echo erLhcoreClassDesign::baseurl('webhooks/edit')?>/<?php echo $item['item']->id?>" target="_blank"><?php echo htmlspecialchars(($item['item']->name ?: 'Webhook #' . $item['item']->id) . ($item['item']->event ? ' | ' . $item['item']->event : ''))?></a>
                                   <span class="badge bg-info me-1"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('bot/conditions','Webhook');?></span>
