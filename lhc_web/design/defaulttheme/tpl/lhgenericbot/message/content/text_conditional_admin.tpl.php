<?php $msgBody = (isset($metaMessage['intro_op']) && !empty($metaMessage['intro_op'])) ? $metaMessage['intro_op'] : $metaMessage['intro_us']; $paramsMessageRender = array('sender' => $msg['user_id']);?>
...
<?php if (!empty($fullMessage)) : ?>
    <?php $msgBody = $fullMessage; $paramsMessageRender = array('sender' => $msg['user_id']);?>
    <button class="btn btn-link fs13 py-0" id="hide-show-action-<?php echo $messageId?>"onclick='lhinst.hideShowAction(<?php echo json_encode(['chat_id' => $msg['chat_id'], 'show_text' => (isset($metaMessage['readmore_op']) && !empty($metaMessage['readmore_op']) ? $metaMessage['readmore_op'] : erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncuser', 'Read more')), 'hide_text' => erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncuser', 'Hide'),'id' => $messageId, 'hide_show' => !isset($metaMessage['hide_show']) || $metaMessage['hide_show'] == true])?>)'>
        <?php if (isset($metaMessage['readmore_op']) && !empty($metaMessage['readmore_op'])) : ?><?php echo htmlspecialchars($metaMessage['readmore_op']); else : ?><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncuser', 'Read more')?><?php endif;?>
    </button>
