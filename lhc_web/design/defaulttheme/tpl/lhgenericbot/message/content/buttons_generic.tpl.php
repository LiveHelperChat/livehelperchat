    ...
    <ul class="bot-btn-list">
        <?php foreach ($metaMessage as $indexSub => $item) : ?>
            <li>
                ...
                <a rel="noreferrer" tabindex="0" data-no-change="true" class="btn-link action-image <?php if (isset($metaMessageData['ch']) && in_array(substr(md5($item['content']['name']),0,16),$metaMessageData['ch'])) : ?>visited<?php endif;?>" <?php if (isset($item['content']['no_name']) && $item['content']['no_name'] == true) : ?>data-no-msg="true"<?php endif; ?> data-id="<?php echo $messageId?>" data-payload=<?php echo json_encode($item['content']['payload']. '__' . substr(md5($item['content']['name']),0,16))?> data-keep="true" onclick='lhinst.updateTriggerClicked(<?php echo json_encode($item['content']['payload'] . '__' . substr(md5($item['content']['name']),0,16))?>,<?php echo $messageId?>,$(this),true)'>
                <?php else : ?>
                <a rel="noreferrer" tabindex="0" data-no-change="true" class="btn-link action-image <?php if (isset($metaMessageData['ch']) && in_array(substr(md5($item['content']['name']),0,16),$metaMessageData['ch'])) : ?>visited<?php endif;?>" <?php if (isset($item['content']['no_name']) && $item['content']['no_name'] == true) : ?>data-no-msg="true"<?php endif; ?> data-id="<?php echo $messageId?>" data-payload=<?php echo json_encode($item['content']['payload']. '__' . substr(md5($item['content']['name']),0,16))?> data-keep="true" onclick='lhinst.buttonClicked(<?php echo json_encode($item['content']['payload'] . '__' . substr(md5($item['content']['name']),0,16))?>,<?php echo $messageId?>,$(this),true)'>
                <?php endif?>
