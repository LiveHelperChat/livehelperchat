<table class="table table-sm mb-0 table-small table-fixed" ng-if="my_mails.list.length > 0">
    ...
    <tr ng-repeat="chat in my_mails.list track by chat.id">
        <td>
            <div class="abbr-list" >
                <i ng-if="chat.status != 1" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Pending chat');?>" class="material-icons chat-unread">&#xE80E;</i><a title="{{chat.id}}" ng-click="lhc.previewMail(chat.id)" class="material-icons">info_outline</a><a ng-click="lhc.startMailChat(chat.id,chat.subject_front)" title="{{chat.from_address}}">{{chat.from_name}} | {{chat.subject_front}}</a><small>{{chat.plain_user_name !== undefined ? ' | ' + chat.plain_user_name : ''}}</small></div>
        </td>
        <td>
