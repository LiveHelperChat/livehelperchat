<div class="card card-dashboard" data-panel-id="online_visitors" ng-init="lhc.getToggleWidget('onvisitors_widget_exp')">
	<div class="card-header">
		<i class="material-icons">&#xf643;</i> <?php include(erLhcoreClassDesign::designtpl('lhfront/dashboard/panels/titles/online_visitors.tpl.php'));?> ({{online.onlineusers.length}})
		<a title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('pagelayout/pagelayout','collapse/expand')?>" ng-click="lhc.toggleWidget('onvisitors_widget_exp')" class="fs24 float-right material-icons exp-cntr">{{lhc.toggleWidgetData['onvisitors_widget_exp'] == false ? '&#xf143;' : '&#xf140;'}}</a>
	</div>
	<div ng-if="lhc.toggleWidgetData['onvisitors_widget_exp'] !== true">
	   ...
	   <div class="panel-list" ng-if="online.onlineusers.length > 0">
		...
		<div ng-if="online.onlineusers.length == 0" class="m-1 alert alert-info"><i class="material-icons">&#xfa3c;</i><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Nothing found')?>...</div>
	</div>
</div>
