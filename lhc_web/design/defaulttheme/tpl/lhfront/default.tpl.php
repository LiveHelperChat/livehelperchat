...
<?php if ($online_chat_enabled_pre == true || $online_visitors_enabled_pre == true) : ?>
    ...
    <div id="old-dashboard" ng-controller="OnlineCtrl as online" ng-init='online.forbiddenVisitors=<?php $currentUser->hasAccessTo('lhchat', 'use_onlineusers') != true ? print 'true' : print 'false'?>;groupByField = <?php echo json_encode($ogroupBy) ?>;online.maxRows="<?php echo (int)$omaxRows ?>";online.time_on_site = <?php echo json_encode($oTimeOnSite)?>;online.country="<?php echo htmlspecialchars($oCountry)?>";online.updateTimeout="<?php echo (int)$oupdTimeout ?>";online.userTimeout = "<?php echo (int)$ouserTimeout ?>";online.department_dpgroups = <?php echo json_encode($onlineDepartmentGroups)?>;online.department=<?php echo json_encode($onlineDepartment)?>;online.soundEnabled=<?php echo $soundUserNotification == 1 ? 'true' : 'false' ?>; online.online_connected=<?php echo $onlineVisitorOnly == 1 ? 'true' : 'false' ?>; online.notificationEnabled=<?php echo $browserNotification == 1 ? 'true' : 'false' ?>'>
