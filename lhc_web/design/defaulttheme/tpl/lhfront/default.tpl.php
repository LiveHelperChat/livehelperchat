...
<?php if ($online_chat_enabled_pre == true || $online_visitors_enabled_pre == true) : ?>
    ...
    <div id="old-dashboard" ng-controller="OnlineCtrl as online" ng-init='online.forbiddenVisitors=<?php $currentUser->hasAccessTo('lhchat', 'use_onlineusers') != true ? print 'true' : print 'false'?>;groupByField = <?php echo json_encode($ogroupBy, JSON_HEX_APOS) ?>;online.maxRows="<?php echo (int)$omaxRows ?>";online.time_on_site = <?php echo json_encode($oTimeOnSite, JSON_HEX_APOS)?>;online.country=<?php echo json_encode($oCountry, JSON_HEX_APOS)?>;online.updateTimeout="<?php echo (int)$oupdTimeout ?>";online.userTimeout = "<?php echo (int)$ouserTimeout ?>";online.department_dpgroups = <?php echo json_encode($onlineDepartmentGroups)?>;online.department=<?php echo json_encode($onlineDepartment, JSON_HEX_APOS)?>;online.soundEnabled=<?php echo $soundUserNotification == 1 ? 'true' : 'false' ?>; online.online_connected=<?php echo $onlineVisitorOnly == 1 ? 'true' : 'false' ?>; online.notificationEnabled=<?php echo $browserNotification == 1 ? 'true' : 'false' ?>'>
