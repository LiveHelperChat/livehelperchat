<tr ng-repeat="ou in group.ou | orderBy:online.predicate:online.reverse | filter:query track by ou.id" id="uo-vid-{{ou.vid}}" class="online-user-filter-row" ng-class="{<?php echo $onlineCheck?>}">
    	...
    	<td>
        	<div ng-if="ou.vid" class="btn-group" role="group" aria-label="...">
                <a href="#" class="btn btn-xs btn-secondary" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('user/account','Copy nick');?>" onclick="lhinst.copyContent($(this))" data-success="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('user/account','Copied');?>" data-copy="{{ou.nick}}"><i class="material-icons me-0">content_copy</i></a>
            ...
            <div style="width:90px" ng-if="ou.vid">
    	        <div class="btn-group" role="group" aria-label="...">
    	            ...
    	            <button ng-click="online.sendMessage(ou.id)" class="btn btn-secondary btn-sm material-icons mat-100 me-0" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/onlineusers','Send message');?>">chat</button>
    	            <?php endif;?>
    	            
    	            <button ng-click="online.deleteUser(ou,'<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('abstract/list','Are you sure?')?>');" class="btn btn-danger btn-sm material-icons mat-100 me-0" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('system/buttons','Delete');?>, ID - {{ou.id}}">delete</button>
    			</div>
    		</div>
            </td>
	</tr>