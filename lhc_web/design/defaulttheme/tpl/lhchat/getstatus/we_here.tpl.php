...
if (!this.cssNHWasAdded) {
	...
	'#lhc_need_help_triangle{width: 0;height: 0;border-left: 20px solid transparent;border-right: 10px solid transparent;border-top: 15px solid #<?php $theme !== false ? print $theme->need_help_bcolor : print '648000' ?>;position:absolute;bottom:-14px;}'+
	'#lhc_need_help_close{cursor:pointer;float:right;border-radius:10px;background:#<?php $theme !== false ? print $theme->need_help_close_bg : print '435A05' ?>;padding:0px 6px;color:#FFF;right:10px;font-size:16px;font-weight:bold;text-decoration:none;margin-top:0px;line-height:20px}#lhc_need_help_close:hover{background-color:#<?php $theme !== false ? print $theme->need_help_close_hover_bg : print '74990F' ?>;}'+
