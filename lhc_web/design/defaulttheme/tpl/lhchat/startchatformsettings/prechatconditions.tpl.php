<script>
    var startChatFieldsConditions = <?php isset($start_chat_data['pre_conditions']) ? print $start_chat_data['pre_conditions'] : print "{'online': [],'maintenance' :[],'offline': [],'disable': [],'offline_enabled' : false,'disable_enabled' : false, 'disable_message' : ''}"; ?>
</script>
...
<div ng-show="preconditions.conditions.maintenance_mode">
    <button type="button" class="btn btn-outline-secondary btn-xs" ng-click="preconditions.addField('maintenance')"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/startchatformsettings','Add condition')?></button><span class="text-muted ps-2"><small><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/startchatformsettings','conditions are optional.')?></small></span>
    <div class="row pt-2" ng-repeat="conditionItem in preconditions.conditions.maintenance track by $index" >
        <div class="col-9">
            <div class="row">
                <div class="col-4">
                    <input class="form-control form-control-sm" ng-model="conditionItem.field" name="field[{{$index}}]" type="text" value="" placeholder="field">
        ...
        <div class="col-3">
            <div class="btn-group" role="group" aria-label="Basic example">
                <button type="button" ng-if="preconditions.conditions.maintenance.length > 0 && preconditions.conditions.maintenance.length != $index + 1" ng-click="preconditions.moveDown(conditionItem,preconditions.conditions.maintenance)" class="btn btn-sm btn-secondary"><i class="material-icons">keyboard_arrow_down</i></button>
                <button type="button" ng-if="$index > 0" ng-click="preconditions.moveUp(conditionItem,preconditions.conditions.maintenance)" class="btn btn-sm btn-secondary"><i class="material-icons">keyboard_arrow_up</i></button>
                <button type="button" ng-click="preconditions.deleteElement(conditionItem,preconditions.conditions.maintenance)" class="btn btn-sm btn-danger"><i class="material-icons me-0">delete</i></button>
            </div>
        </div>
    </div>
    <div class="pt-2">
                    <span ng-repeat="transactionItem in preconditions.conditions.maintenance track by $index">
                        {{((transactionItem.logic == 'or') && ($index == 0 || preconditions.conditions.maintenance[$index - 1].logic == 'and' || !preconditions.conditions.maintenance[$index - 1].logic)) ? ' ( ' : ''}}<span class="badge" ng-class="{'bg-success':!transactionItem.exclude,'bg-danger':transactionItem.exclude}">{{$index + 1}}.</span>{{transactionItem.logic == 'and' && (preconditions.conditions.maintenance[$index - 1].logic == 'or') ? ' ) ' : ''}}
                        {{(transactionItem.logic == 'or') ? ' or ' : (($index+1 != preconditions.conditions.maintenance.length) ? ' and ' : '')}}
                    </span>
        <span class="mt-1 mb-1 p-2 badge fs14 d-block bg-warning"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/startchatformsettings','Maintenance mode activated')?></span>
    </div>
