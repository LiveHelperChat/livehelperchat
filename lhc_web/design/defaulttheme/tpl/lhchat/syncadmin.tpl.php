...
<div class="message-row<?php echo $msg['user_id'] == 0 ? ' response' : ' message-admin'.($lastOperatorChanged == true ? ' operator-changes' : '') ?><?php echo $otherOperator == true ? ' other-operator' : ''?>" data-op-id="<?php echo $msg['user_id']?>" title="<?php echo erLhcoreClassChat::formatDate($msg['time']);?>" id="msg-<?php echo $msg['id']?>">
    <span class="usr-tit <?php echo $msg['user_id'] == 0 ? ' vis-tit' : ' op-tit'?>"><?php if ($msg['user_id'] != 0) : ?><span class="msg-date text-muted font-weight-normal pr-2"><?php echo erLhcoreClassChat::formatDate($msg['time']);?></span><?php endif;?><?php echo $msg['user_id'] == 0 ? '<i class="material-icons chat-operators mi-fs15 mr-0 pb-1">'.($chat->device_type == 0 ? '&#xE30A;' : ($chat->device_type == 1 ? '&#xE32C;' : '&#xE32F;')).'</i> '.htmlspecialchars($lhcNickAlias ?? $chat->nick) : '<i class="material-icons chat-operators mi-fs15 mr-0 pb-1">&#xE851;</i>'.htmlspecialchars($msg['name_support']) ?><?php if ($msg['user_id'] == 0) : ?><span class="pl-2 msg-date text-muted font-weight-normal"><?php echo erLhcoreClassChat::formatDate($msg['time']);?></span><?php endif;?></span>
