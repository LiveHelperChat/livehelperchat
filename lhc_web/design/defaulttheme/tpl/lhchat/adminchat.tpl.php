<div class="row">
	<div class="col-md-8 ps-0 chat-main-left-column" id="chat-main-column-<?php echo $chat->id;?>">
		...
		<div class="message-container-admin" >
		    ...
		    <textarea <?php if (!erLhcoreClassUser::instance()->hasAccessTo('lhmailconv','closerelated')) : ?>close-related="false"<?php endif; ?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','editpreviousall')) : ?>edit-all="true"<?php endif;?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','editpreviouvis')) : ?>edit-vis="true"<?php endif;?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','editpreviousop')) : ?>edit-op="true"<?php endif;?> <?php if (!erLhcoreClassUser::instance()->hasAccessTo('lhchat','editprevious')) : ?>disable-edit="true"<?php endif;?> <?php if ($whisperMode) : ?>whisper="1"<?php endif;?> <?php !erLhcoreClassChat::hasAccessToWrite($chat) || ($chat->status != erLhcoreClassModelChat::STATUS_OPERATORS_CHAT && $chat->user_id != 0 && $chat->user_id != erLhcoreClassUser::instance()->getUserID() && !erLhcoreClassUser::instance()->hasAccessTo('lhchat','writeremotechat') && $writeRemoteDisabled = true) ? print 'readonly="readonly"'  : '' ?> title="<?php echo $placeholderValue?>" placeholder="<?php echo $placeholderValue?>" class="form-control form-control-sm form-send-textarea form-group<?php if ($chat->user_id != erLhcoreClassUser::instance()->getUserID()) : ?> form-control-warning<?php endif;?>" data-rows-default="<?php echo (int)erLhcoreClassModelUserSetting::getSetting('chat_text_rows',2)?>" rows="<?php echo (int)erLhcoreClassModelUserSetting::getSetting('chat_text_rows',2)?>" <?php if ($chat->status == erLhcoreClassModelChat::STATUS_CLOSED_CHAT) : ?>readonly="readonly"<?php endif;?> name="ChatMessage" id="CSChatMessage-<?php echo $chat->id?>"></textarea>
