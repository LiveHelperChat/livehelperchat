<div class="row">
	<div class="col-md-8 ps-0 chat-main-left-column" id="chat-main-column-<?php echo $chat->id;?>">
		...
		<div class="message-container-admin" >
            ...
            <?php if ( isset($fileData['active_admin_upload']) && $fileData['active_admin_upload'] == true && erLhcoreClassUser::instance()->hasAccessTo('lhfile','use_operator') ) : $filesEnabled = true;?>
                ...
                <script>
                    var file_upload_<?php echo $chat->id?> = {ft_msg:'<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('files/files','Not an accepted file type')?>',fs_msg:'<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('files/files','Filesize is too big')?>',chat_id:'<?php echo $chat->id?>',fs:<?php echo $fileData['fs_max']*1024?>,ft_op:/(\.|\/)(<?php echo $fileData['ft_op']?>)$/i};
                    <?php if ((int)erLhcoreClassModelUserSetting::getSetting('new_editor', 0) == 0) : ?>
                    lhinst.addFileUpload(file_upload_<?php echo $chat->id?>);
                    <?php endif; ?>
                </script>
            <?php endif; endif;?>
            ...
            <?php include(erLhcoreClassDesign::designtpl('lhchat/part/before_text_area_block_multiinclude.tpl.php')); ?>

            <?php if ((int)erLhcoreClassModelUserSetting::getSetting('new_editor', 0) == 0) : ?>
                <textarea <?php if (!erLhcoreClassUser::instance()->hasAccessTo('lhmailconv','closerelated')) : ?>close-related="false"<?php endif; ?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','editpreviousall')) : ?>edit-all="true"<?php endif;?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','removemsgvi')) : ?>remove-msg-vi="true"<?php endif;?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','removemsgop')) : ?>remove-msg-op="true"<?php endif;?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','editpreviouvis')) : ?>edit-vis="true"<?php endif;?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','editpreviousop')) : ?>edit-op="true"<?php endif;?> <?php if (!erLhcoreClassUser::instance()->hasAccessTo('lhchat','editprevious')) : ?>disable-edit="true"<?php endif;?> <?php if ($whisperMode) : ?>whisper="1"<?php endif;?> <?php !erLhcoreClassChat::hasAccessToWrite($chat) || ($chat->status != erLhcoreClassModelChat::STATUS_OPERATORS_CHAT && $chat->user_id != 0 && $chat->user_id != erLhcoreClassUser::instance()->getUserID() && !erLhcoreClassUser::instance()->hasAccessTo('lhchat','writeremotechat') && $writeRemoteDisabled = true) ? print 'readonly="readonly"'  : '' ?> title="<?php echo $placeholderValue?>" placeholder="<?php echo $placeholderValue?>" class="form-control form-control-sm form-send-textarea form-group<?php if ($chat->user_id != erLhcoreClassUser::instance()->getUserID()) : ?> form-control-warning<?php endif;?>" data-rows-default="<?php echo (int)erLhcoreClassModelUserSetting::getSetting('chat_text_rows',2)?>" rows="<?php echo (int)erLhcoreClassModelUserSetting::getSetting('chat_text_rows',2)?>" <?php if ($chat->status == erLhcoreClassModelChat::STATUS_CLOSED_CHAT) : ?>readonly="readonly"<?php endif;?> name="ChatMessage" id="CSChatMessage-<?php echo $chat->id?>"></textarea>
            <?php else : ?>
            <div class="pb-2" style="min-height: <?php echo (int)erLhcoreClassModelUserSetting::getSetting('chat_text_rows',2)*31; ?>px">
                <lhc-editor <?php if (!(!isset($bbcodeOptions['disabled_options']) || !in_array('[u]',$bbcodeOptions['disabled_options']))) : ?>disable_bb_underline="true"<?php endif;?> <?php if (!(!isset($bbcodeOptions['disabled_options']) || !in_array('[i]',$bbcodeOptions['disabled_options']))) : ?>disable_bb_italic="true"<?php endif;?> <?php if (!(!isset($bbcodeOptions['disabled_options']) || !in_array('[b]',$bbcodeOptions['disabled_options']))) : ?>disable_bb_bold="true"<?php endif;?> enable_canned_suggester="true" scope="chat" <?php if (!erLhcoreClassUser::instance()->hasAccessTo('lhmailconv','closerelated')) : ?>close-related="false"<?php endif; ?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','editpreviousall')) : ?>edit-all="true"<?php endif;?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','removemsgvi')) : ?>remove-msg-vi="true"<?php endif;?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','removemsgop')) : ?>remove-msg-op="true"<?php endif;?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','editpreviouvis')) : ?>edit-vis="true"<?php endif;?> <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhchat','editpreviousop')) : ?>edit-op="true"<?php endif;?> <?php if (!erLhcoreClassUser::instance()->hasAccessTo('lhchat','editprevious')) : ?>disable-edit="true"<?php endif;?> <?php if ($whisperMode) : ?>whisper="1"<?php endif;?> <?php !erLhcoreClassChat::hasAccessToWrite($chat) || ($chat->status != erLhcoreClassModelChat::STATUS_OPERATORS_CHAT && $chat->user_id != 0 && $chat->user_id != erLhcoreClassUser::instance()->getUserID() && !erLhcoreClassUser::instance()->hasAccessTo('lhchat','writeremotechat') && $writeRemoteDisabled = true) ? print 'readonly="readonly"'  : '' ?> title="<?php echo $placeholderValue?>" placeholder="<?php echo $placeholderValue?>" <?php if ($chat->user_id != erLhcoreClassUser::instance()->getUserID()) : ?>warning_area="true"<?php endif;?>" data_rows_default="<?php echo (int)erLhcoreClassModelUserSetting::getSetting('chat_text_rows',2)?>" rows="<?php echo (int)erLhcoreClassModelUserSetting::getSetting('chat_text_rows',2)?>" <?php if ($chat->status == erLhcoreClassModelChat::STATUS_CLOSED_CHAT) : ?>readonly="readonly"<?php endif;?> record_id="<?php echo $chat->id?>" id="CSChatMessage-<?php echo $chat->id?>"></lhc-editor>
            </div>
            <?php endif; ?>
	...
	<div class="col-md-4 chat-main-right-column mh150" translate="no" id="chat-right-column-<?php echo $chat->id;?>">
