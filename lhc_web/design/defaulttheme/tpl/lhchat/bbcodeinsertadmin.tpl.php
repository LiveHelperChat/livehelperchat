...
<script>$('.bb-list a').click(function(){
	...
	var caretPos = document.getElementById("CSChatMessage-"+chatid).selectionStart;
    var textAreaTxt = jQuery("#CSChatMessage-"+chatid).val();
    ...
    if (typeof $(this).attr('data-promt') != 'undefined' && $(this).attr('data-promt') == 'img') {
    	...
    	if (link) {
    		...
    		jQuery("#CSChatMessage-"+chatid).val(textAreaTxt.substring(0, caretPos) + txtToAdd + textAreaTxt.substring(caretPos) );   
    	    $('#myModal').modal('hide');
        }
