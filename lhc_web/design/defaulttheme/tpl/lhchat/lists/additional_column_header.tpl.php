<th width="20%" ng-repeat="column in lhc.additionalColumns" ng-if="column.cenabl == true && !column.iconm">
    ...
    <a ng-if="column.sorten" ng-click="lhc.toggleWidgetSort('<?php echo $additionalChatColumnOptions['sort_field']?>',column.items[0] + '_dsc', column.items[0] + '_asc',true)">
        <i ng-class="{'text-muted' : (lhc.toggleWidgetData['<?php echo $additionalChatColumnOptions['sort_field']?>'] != column.items[0] + '_asc' && lhc.toggleWidgetData['<?php echo $additionalChatColumnOptions['sort_field']?>'] != column.items[0] + '_dsc')}" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Sort by location')?>" class="material-icons">{{lhc.toggleWidgetData['<?php echo $additionalChatColumnOptions['sort_field']?>'] == column.items[0] + '_dsc' || lhc.toggleWidgetData['<?php echo $additionalChatColumnOptions['sort_field']?>'] != column.items[0] + '_asc' ? 'trending_up' : 'trending_down'}}</i>
    </a>
