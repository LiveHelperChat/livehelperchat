...
<ul class="no-bullet small-list">
	<?php foreach ($chats as $chat) : ?>
		...
		<a class="material-icons" title="ID - <?php echo $chat->id;?>" onclick="lhc.previewChat('<?php echo $chat->id;?>')" >info_outline</a>
		<a class="right-action-hide material-icons" data-title="<?php echo erLhcoreClassDesign::shrt($chat->nick,10,'...',30,ENT_QUOTES);?>" onclick="lhinst.startChat('<?php echo $chat->id;?>',$('#tabs'),$(this).attr('data-title'))" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmininterface','Add chat');?>">chat</a>
		...
		<?php endif; ?>
		<?php echo htmlspecialchars($chat->nick);?>, <?php echo $chat->time_created_front;?>, <?php echo htmlspecialchars($chat->department);?> | <?php
