...
<?php foreach ($subMessages as $subMessage) : ?>
    ...
    <div class="msg-body<?php (in_array('img',$subMessage['flags']))? print ' msg-body-media' : ''?><?php (in_array('emoji',$subMessage['flags']))? print ' msg-body-emoji' : ''?><?php if (isset($paramsMessageRender['msg_body_class']) && !empty($paramsMessageRender['msg_body_class'])) : ?><?php echo ' '.$paramsMessageRender['msg_body_class']?><?php endif; ?>">
        ...
        <?php endif; ?>
            ...
            <?php if ( isset($msg) && isset($msg['del_st']) && is_array($msg) && !isset($hideDeliveryStatus) &&
                (
                    ((!isset($visitorRender) || $visitorRender !== true) &&  ($msg['user_id'] > 0 || $msg['user_id'] == -2)) ||
                    (isset($visitorRender) && $visitorRender == true && $msg['user_id'] == 0)
                )
            ) : // Render status only for admin/user messages and bot?>
                <?php if ($msg['del_st'] == erLhcoreClassModelmsg::STATUS_PENDING) : ?>
                    <span title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmin','Scheduled for sent!')?>" class="material-icons me-0 text-warning msg-del-st-<?php echo erLhcoreClassModelmsg::STATUS_PENDING;?>">radio_button_unchecked</span>
                <?php elseif ($msg['del_st'] == erLhcoreClassModelmsg::STATUS_SENT) : ?>
                    <span title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmin','Have been sent!')?>" class="material-icons me-0 <?php if (!isset($visitorRender) || $visitorRender == false) : ?>text-success<?php endif;?> msg-del-st-<?php echo erLhcoreClassModelmsg::STATUS_SENT;?>"><?php if (isset($visitorRender) && $visitorRender == true) : ?>&#xf129;<?php else : ?>done<?php endif; ?></span>
                <?php elseif ($msg['del_st'] == erLhcoreClassModelmsg::STATUS_DELIVERED) : ?>
                    <span title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmin','Delivered')?>" class="material-icons me-0 <?php if (!isset($visitorRender) || $visitorRender == false) : ?>text-success<?php endif;?> msg-del-st-<?php echo erLhcoreClassModelmsg::STATUS_DELIVERED;?>"><?php if (isset($visitorRender) && $visitorRender == true) : ?>&#xf12a;<?php else : ?>done_all<?php endif; ?></span>
                <?php elseif ($msg['del_st'] == erLhcoreClassModelmsg::STATUS_READ && isset($keepDeliveredTick)) : ?>
                        <span title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmin','Have been seen!')?>" class="material-icons me-0 text-primary msg-del-st-<?php echo erLhcoreClassModelmsg::STATUS_READ;?>">done_all</span>
                    <?php elseif ($msg['del_st'] == erLhcoreClassModelmsg::STATUS_REJECTED) : ?>
                    <span title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/syncadmin','Rejected or failed!')?>" class="material-icons me-0 text-danger msg-del-st-<?php echo erLhcoreClassModelmsg::STATUS_REJECTED;?>">unpublished</span>
                <?php endif; ?>
            <?php endif; ?>
        ...
        <?php if ((!isset($visitorRender) || $visitorRender !== true) &&
            ...
            isset($metaMessageData['content']['close_reason']) ||
