<div class="mt-2">
    ...
    <?php foreach (erLhcoreClassModelDepartamentGroup::getList() as $depGroupItem) : $idsUnique = [];?>
        <h6>
            <input type="button" class="btn btn-xs btn-secondary" value="Show/Hide" onclick="$('#dep-group-filter-id-<?php echo $depGroupItem->id?>').toggle()">
            <?php echo htmlspecialchars($depGroupItem->name)?>
        </h6>
        <div id="dep-group-filter-id-<?php echo $depGroupItem->id?>" style="display: none">
            ...
            <?php if (!empty($idsUnique)) : ?>
                <label><b><?php echo htmlspecialchars($depGroupItem->name)?></b></label>
    ...
    <?php if (!empty($memberOf)) : ?>
        <h6 class="border-top pt-2">
            <input type="button" class="btn btn-xs btn-secondary" value="Show/Hide" onclick="$('.member-of-dep-filter').toggle()">
            <?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/transferchat','Member of these departments groups')?>
            </h6>
        </h6>
    <?php endif; ?>
    ...
    <h6 class="border-top pt-2">
        <input type="button" class="btn btn-xs btn-secondary" value="Show/Hide" onclick="$('#online-transfer-dep').toggle()">
        <span class="label label-success"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/transferchat','Online');?></span>
        </h6>
    ...
    <h6 class="border-top pt-2">
        <input type="button" class="btn btn-xs btn-secondary" value="Show/Hide" onclick="$('#offline-transfer-dep').toggle()">
        <span class="label label-default"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/transferchat','Offline');?></span>
