            ...
            <div class="col-6 pb-1">
                <div class="text-muted" id="chat-owner-<?php echo $chat->id?>" user-id="<?php echo $chat->user_id?>">
                    <?php $user = $chat->getChatOwner();  if ($user !== false) : ?>
                        <span class="material-icons action-image" onclick="lhc.revealModal({'url':WWW_DIR_JAVASCRIPT+'chat/chathistory/<?php echo $chat->id?>'})" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/adminchat','Operator chats during this conversations.')?>">history</span><?php echo htmlspecialchars($user->name.' '.$user->surname)?>
                    <?php endif; ?>
                </div>
            </div>
