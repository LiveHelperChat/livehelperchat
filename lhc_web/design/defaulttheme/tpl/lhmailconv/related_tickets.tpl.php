<?php if (!empty($mails)) : ?>
    <ul class="fs14 list-unstyled mb-0" style="max-height: 108px;overflow-y: auto;">
        <?php foreach ($mails as $mail) : ?>
            <li class="text-muted <?php if (!erLhcoreClassChat::hasAccessToWrite($mail)) : ?>opacity-50<?php endif;?>">
                <input class="mr-1" type="checkbox" <?php if (!erLhcoreClassChat::hasAccessToWrite($mail)) : ?>disabled="disabled" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/adminchat','You do not have permission to close conversation')?>" <?php else : ?>title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/adminchat','Check e-email for close on ticket close')?>"<?php endif;?> onchange="lhinst.setAreaAttrByCheckbox(<?php echo $chat->id?>,'closemailconfirm');" name="closemailconfirm-<?php echo $chat->id?>" value="<?php echo $mail->id?>" <?php if (
                           (isset($related_actions['closemail']) && is_array($related_actions['closemail']) && in_array($mail->id,$related_actions['closemail'])) ||
                           (!isset($related_actions['closemail']) && $mail->has_attachment == erLhcoreClassModelMailconvConversation::ATTACHMENT_EMPTY)
    ...
    <script>lhinst.setAreaAttrByCheckbox(<?php echo $chat->id?>,'closemailconfirm');</script>
<?php else : ?>
