    ...
    <ul class="fs14 list-unstyled mb-0" style="max-height: 108px;overflow-y: auto;">
        <?php foreach ($mails as $mail) : ?>
            <li class="text-muted <?php if (!(erLhcoreClassChat::hasAccessToRead($mail) && erLhcoreClassChat::hasAccessToWrite($mail))) : ?>opacity-50<?php endif;?>">
                ...
                <span class="material-icons user-select-none">mail</span><span><?php echo $mail->id?></span><span class="ms-1 me-1 badge <?php if ($mail->status == erLhcoreClassModelMailconvConversation::STATUS_PENDING) : ?>badge-warning<?php else : ?>bg-success<?php endif; ?>">
