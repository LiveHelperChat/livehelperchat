...
<ul class="fs14 list-unstyled mb-0" style="max-height: 108px;overflow-y: auto;">
    <?php foreach ($mails as $mail) : ?>
        <li class="text-muted <?php if (!erLhcoreClassChat::hasAccessToWrite($mail)) : ?>opacity-50<?php endif;?>">
            <input type="checkbox" class="mr-1" <?php if (!erLhcoreClassChat::hasAccessToWrite($mail)) : ?>disabled="disabled" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/adminchat','You do not have permission to close conversation')?>" <?php else : ?>title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/adminchat','Check e-email for close on ticket close')?>" <?php endif; ?> onchange="lhinst.setAreaAttrByCheckbox(<?php echo $chat->id?>,'closemail');" name="closemail-<?php echo $chat->id?>" value="<?php echo $mail->id?>" <?php if ($mail->has_attachment == erLhcoreClassModelMailconvConversation::ATTACHMENT_EMPTY) : ?>checked="checked"<?php endif; ?>  />
