...
foreach ($messages as $msg) : $msgBody = $msg['msg']; ?>
            ...
            <?php } else { ?>
                ...
                <div class="message-row chatbox-row chatbox-row-response"><span class="usr-tit" data-sender="<?php echo htmlspecialchars($chat->nick)?>"><?php echo htmlspecialchars($chat->nick)?><div class="msg-date"><?php if (date('Ymd') == date('Ymd',$msg['time'])) { echo  date(erLhcoreClassModule::$dateHourFormat,$msg['time']);} else {	echo date(erLhcoreClassModule::$dateDateHourFormat,$msg['time']);}; ?></div></span>
