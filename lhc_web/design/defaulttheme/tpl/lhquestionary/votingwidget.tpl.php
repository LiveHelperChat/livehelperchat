...
<?php if ($voting->is_voting == 1) : ?>
	...
	<?php endforeach;?>
	<br>
	<input type="submit" class="small button" value="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('questionary/votingwidget','Vote');?>" name="VoteAction">
	<?php else : ?>
	<textarea name="feedBack"><?php echo htmlspecialchars($answer->answer)?></textarea>

	<input type="submit" class="small button" value="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('questionary/votingwidget','Send');?>" name="FeedBackAction">
	<?php endif;?>
