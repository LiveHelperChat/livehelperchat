...
const StartChat = React.lazy(() => import('./StartChat'));
const OnlineChat = React.lazy(() => import('./OnlineChat'));
const HeaderChat = React.lazy(() => import('./HeaderChat'));
...
class BodyChat extends Component {
    ...
    render() {
        ...
        if (this.props.chatwidget.get('isChatting') === true) {
            ...
            return (<React.Fragment>{this.props.chatwidget.hasIn(['chat_ui','custom_html_header']) && <div className="lhc-custom-header-above" dangerouslySetInnerHTML={{__html:this.props.chatwidget.getIn(['chat_ui','custom_html_header'])}}></div>}{this.props.chatwidget.get('mode') == 'widget' && <Suspense fallback=""><HeaderChat popupChat={this.popupChat} endChat={this.endChat} /></Suspense>}<div className={className}><Suspense fallback=""><OnlineChat hideMessageField={this.hideMessageField} profileBefore={this.profileHTML} messagesBefore={this.messagesHTML} cancelClose={this.cancelClose} endChat={this.endChat} /></Suspense></div></React.Fragment>)
        } else if (this.props.chatwidget.get('isOnline') === true && this.props.chatwidget.get('isOfflineMode') === false) {
            className += " start-chat";
            return (<React.Fragment>{this.props.chatwidget.hasIn(['chat_ui','custom_html_header']) && <div className="lhc-custom-header-above" dangerouslySetInnerHTML={{__html:this.props.chatwidget.getIn(['chat_ui','custom_html_header'])}}></div>}{this.props.chatwidget.get('mode') == 'widget' && <Suspense fallback=""><HeaderChat popupChat={this.popupChat} endChat={this.endChat} /></Suspense>}<div className={className}><Suspense fallback=""><StartChat botPayload={this.botPayload} setHideMessageField={this.setHideMessageField} setProfile={this.setProfile} setMessages={this.setMessages} /></Suspense></div></React.Fragment>)
        } else {
            className += " offline-chat";
            return (<React.Fragment>{this.props.chatwidget.hasIn(['chat_ui','custom_html_header']) && <div className="lhc-custom-header-above" dangerouslySetInnerHTML={{__html:this.props.chatwidget.getIn(['chat_ui','custom_html_header'])}}></div>}{this.props.chatwidget.get('mode') == 'widget' && <Suspense fallback=""><HeaderChat popupChat={this.popupChat} endChat={this.endChat} /></Suspense>}<div className={className}><Suspense fallback=""><OfflineChat /></Suspense></div></React.Fragment>)
        }
    }
}
