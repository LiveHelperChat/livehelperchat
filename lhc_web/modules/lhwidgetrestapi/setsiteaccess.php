...
erLhcoreClassChatEventDispatcher::getInstance()->dispatch('chat.validstatus_chat', ['chat' => & $chat, 'valid_statuses' => & $validStatuses]);
...
if ($chat->hash === $payload['hash'] && (in_array($chat->status,$validStatuses)) && isset($payload['lng']) && in_array($payload['lng'], erConfigClassLhConfig::getInstance()->getSetting('site', 'available_site_access')))
{
    ...
    $chat->updateThis(['update' => ['chat_locale']]);
}

echo erLhcoreClassChat::safe_json_encode(['error' =>  false]);
exit;
