...
$tpl = erLhcoreClassTemplate::getInstance( 'lhnotifications/oplist.tpl.php');
...
$pages->items_total = \LiveHelperChat\Models\Notifications\OperatorSubscriber::getCount($filterParams['filter']);
$pages->translationContext = 'chat/pendingchats';
$pages->serverURL = erLhcoreClassDesign::baseurl('notifications/oplist').$append;
$pages->paginate();
$tpl->set('pages',$pages);

if ($pages->items_total > 0) {
    $items = \LiveHelperChat\Models\Notifications\OperatorSubscriber::getList(array_merge($filterParams['filter'],array('limit' => $pages->items_per_page,'offset' => $pages->low)));
    $tpl->set('items',$items);
}

$filterParams['input_form']->form_action = erLhcoreClassDesign::baseurl('notifications/oplist');
