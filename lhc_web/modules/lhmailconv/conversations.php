...
if ( isset($_POST['doClose']) ) {
    ...
    if ( $form->hasValidData( 'ConversationID' ) && !empty($form->ConversationID) ) {
        ...
        foreach ($chats as $chatToClose) {
            if ($currentUser->hasAccessTo('lhmailconv','close_all_conversation') || (erLhcoreClassChat::hasAccessToWrite($chatToClose) && erLhcoreClassChat::hasAccessToRead($chatToClose)) )
            {
